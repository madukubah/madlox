...
// forStatement    | "for" "(" (varDecl | exprStatement | ";") expr? ";" expr? ")" statement
...
static void initCompiler(Compiler* compiler, FunctinType type){
...
static int resolveLocal(Compiler* compiler, Token* name){
    for(int i = compiler->localCount - 1; i >= 0; i--){
        ...
        if(identifiersEqual(name, &local->name)){
            if(local->depth == -1){
                error("can`t read initializer with its own initializer.");
            }
            return i;
        }
    }
...
static void function(FunctinType type){
    Compiler compiler;
    ...
    consume(TOKEN_LEFT_BRACE, "Expect '{' before body function.");
    block();
...
static void forStatement(){
    ...
    if(!match(TOKEN_RIGHT_PAREN)){
        ...
        int increementStart = currentChunk()->count;
...
static void varDeclaration(){
    ...
    consume(TOKEN_SEMICOLON, "Exprect ';' after variable declaration.");
